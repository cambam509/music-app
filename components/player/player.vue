<template>
  <section v-if="allMusic">
    <div class="container">
      <div class="row mb-5">
        <div class="col-md-12">
          <h3 class="text-center mt-5">Audio Player</h3>
          <h6 class="text-center mt-4 text-info">Please Select a Song to Play</h6>
        </div>
      </div>
      <div class="row mt-5">
        <div class="col-md-6">
          <img
            src="https://thumbs.gfycat.com/SorrowfulHalfAntlion-size_restricted.gif"
            class="image"
          />
          <div class="card player_card">
            <div class="card-body">
              <h6 class="card-title">
                <b class="text-info">{{this.current.title}} - {{this.current.artist}}</b>
              </h6>
              <div>
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(music,index) in allMusic" :key="index">
                  ...
                  <td>{{music.artist}}</td>
                  <td>
                    <button class="btn btn-info" @click="play(music)">Play</button>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  methods: {
    ...
    play(song) {
      console.log('Current song playing: ', song);
      if (song) {
        this.current = song;
        this.player.src = `http://localhost:4000/${this.current.music.path}`
      }
